...
此版本整合关系图自适应布局、完整关系标签、框架与行动规则的关系复核、PDF 原文标题索引，以及逐单元行动规则账本。核心框架悬浮详情的长关系列表保留内部滚动条：鼠标位于列表上时先滚动列表，到边界后再滚动外层浮窗；行动规则浮窗原有滚动方式保持不变。以下按功能介绍，早期内部试验编号不作为公开版本号。

### 关系卡片文字完整显示

关系图卡片底部的关系名称与目标名称不再单行省略，而是在卡片内换行显示。现有动态高度测量会随卡片变高重新排布下方节点和箭头；“适合宽度”与“查看全图”仍分别处理阅读与全局浏览。验收要求覆盖长关系名称、窄屏和放大文字。

### 关系图布局与交互美术

两张关系图共用自适应列布局、较高的画图区和紧凑卡片。工具栏将“适合宽度”与“查看全图”分开；普通缩放保持当前视野中心。相邻层的箭头走短路线，跨层及反馈边使用外围通道。动效为首次轻量呈现和选中关系时的一次方向提示。浮层支持慢速移入保护与内部滚动。
...
悬浮详情面板（`diagram-hovercard`）在内容过长时可滚动，鼠标从卡片慢速移入浮窗不会立即关闭。核心框架的关系条目列表若出现自己的滚动条，鼠标在列表上滚动时先移动列表，余量再交给外层浮窗；鼠标在浮窗其他位置时直接滚动外层。行动规则浮窗使用外层滚动。浮窗阅读期间不会带动背景页面。
